...
function iconoRed(red) {
  ...
  if (red === 'WhatsApp') return (<svg {...props}><path d="M12 2a10 10 0 00-8.6 15l-1.3 4.7 4.8-1.3A10 10 0 1012 2m0 1.8a8.2 8.2 0 11-4.2 15.2l-.3-.2-2.8.8.8-2.8-.2-.3A8.2 8.2 0 0112 3.8m4.7 10.3c-.3-.1-1.5-.7-1.7-.8-.2-.1-.4-.1-.6.1-.2.3-.6.8-.8 1-.1.1-.3.2-.5 0-.3-.1-1.1-.4-2-1.2-.8-.7-1.3-1.5-1.4-1.8-.1-.2 0-.4.1-.5l.4-.4c.1-.2.2-.3.2-.5.1-.2 0-.3 0-.5s-.6-1.4-.8-1.9c-.2-.5-.4-.4-.6-.4h-.5c-.2 0-.5.1-.7.3-.3.3-1 .9-1 2.3s1 2.7 1.2 2.9c.1.2 2 3 4.8 4.2.7.3 1.2.5 1.6.6.7.2 1.3.2 1.8.1.5-.1 1.5-.6 1.7-1.2.2-.6.2-1.1.2-1.2-.1-.1-.3-.2-.5-.3" /></svg>)
...
function CatalogPage() {
  ...
  const redes = [
    { user: tenant?.whatsapp_number, base: 'https://wa.me/', label: 'WhatsApp', esWhats: true },
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: secondary, fontFamily: fontBody }}>
      ...
      <header className="shadow-sm relative" style={{ backgroundColor: primary }}>
        {headerImage && (
          <div className="absolute inset-0 overflow-hidden">
            <div className="absolute inset-0" style={{ backgroundImage: `url(${headerImage})`, backgroundSize: 'cover', backgroundPosition: 'center' }}></div>
        ...
        <div className="max-w-5xl mx-auto p-6 relative">
          ...
          <div className="absolute top-4 right-4 z-30">
            {user ? (
              <div className="relative">
                ...
                {accountMenu && (
                  <>
                    <div className="fixed inset-0 z-40" onClick={() => setAccountMenu(false)}></div>
          ...
          ) : tenant?.theme?.logo_layout === 'logo-only' ? (
            <div className="flex flex-col items-center text-center gap-2">
              {tenant?.theme?.logo_url
                ? <img src={tenant.theme.logo_url} alt={tenant.name} className="w-20 h-20 rounded-full object-cover border-2 border-white" />
                : <h1 className="text-3xl font-bold text-white" style={{ fontFamily: fontTitle }}>{tenant.name}</h1>}
              {headerText && <p className="text-white/80">{headerText}</p>}
            </div>
          ) : tenant?.theme?.logo_layout === 'name-only' ? (
            <div className="text-center">
              <h1 className="text-4xl font-bold text-white" style={{ fontFamily: fontTitle }}>{tenant.name}</h1>
              {headerText && <p className="text-white/80 mt-1">{headerText}</p>}
            </div>
          ) : tenant?.theme?.logo_layout === 'center-inline' ? (
          ...
          ) : tenant?.theme?.logo_layout === 'name-big' ? (
            <div className="flex items-center justify-between gap-3">
              <div>
                <h1 className="text-4xl font-bold text-white" style={{ fontFamily: fontTitle }}>{tenant.name}</h1>
                {headerText && <p className="text-white/80">{headerText}</p>}
              </div>
              {tenant?.theme?.logo_url && <img src={tenant.theme.logo_url} alt={tenant.name} className="w-14 h-14 rounded-full object-cover border-2 border-white flex-shrink-0" />}
            </div>
          ) : (
            <div className="flex items-center gap-3">
              {tenant?.theme?.logo_url && <img src={tenant.theme.logo_url} alt={tenant.name} className="w-14 h-14 rounded-full object-cover border-2 border-white" />}
      ...
      <main className="max-w-5xl mx-auto p-6">
                    ...
                    return (
                      <div key={prod.id} onClick={irADetalle} className={`bg-white rounded-lg shadow-sm overflow-hidden ${clickable} ${!prod.is_available ? 'opacity-60' : ''}`}>
                        ...
                        <div className="p-4">
                          ...
                          <p className="text-lg font-bold" style={{ color: primary }}>${Number(prod.base_price).toFixed(2)}</p>
                        </div>
                      </div>
                    )
                  }
                    ...
                    return (
                      <div key={prod.id} onClick={irADetalle} className={`bg-white rounded-lg shadow-sm p-3 flex gap-3 items-center ${clickable} ${!prod.is_available ? 'opacity-60' : ''}`}>
                        ...
                        <div className="flex-1">
                          ...
                          <p className="font-bold mt-1" style={{ color: primary }}>${Number(prod.base_price).toFixed(2)}</p>
                        </div>
                      </div>
                    )
                  }
        ...
        <div className="max-w-5xl mx-auto px-6 py-10">
          <p className="text-center text-sm text-gray-500 mb-4" style={{ fontFamily: fontTitle }}>Cómo contactarnos</p>
          <div className="flex justify-center gap-8 flex-wrap">
            {redes.map((r) => (
              <a key={r.label} href={`${r.base}${r.user}`} target="_blank" rel="noopener noreferrer"
                ...
                <span className="text-xs text-gray-600">{r.esWhats ? 'WhatsApp' : `@${r.user}`}</span>
              </a>
            ))}
          </div>
        </div>

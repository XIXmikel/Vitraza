    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
      <div className="bg-white p-8 rounded-lg shadow-sm w-full max-w-sm">
        <div className="flex justify-center mb-6">
          <img src="/vitraza-logo.png" alt="Vitraza" className="h-12" />
        </div>
        ...
        <input type="password" value={password} onChange={(e) => setPassword(e.target.value)}
          className="w-full border border-gray-300 rounded px-3 py-2 mb-6" placeholder="Mínimo 6 caracteres" />
        ...
        <p className="text-sm text-gray-500 text-center mt-4">
          ¿Ya tienes cuenta?{' '}
          <Link to={`/login${window.location.search}`} className="text-blue-600 hover:underline">Inicia sesión</Link>
        </p>
      </div>
    </div>
  )
